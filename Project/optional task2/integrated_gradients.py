...
if __name__ == "__main__":
    ...
    output /= output.max()
    cv2.imwrite('./assets/integrated_gradients_resnet34_fireboat.png', np.uint8((output.permute(1, 2, 0).cpu().numpy() + 0.1) * image_np))

    output = ig.attribute(input_tensor2.unsqueeze(0))
    output = output.squeeze(0).abs()
    output /= output.max()
    cv2.imwrite('./assets/integrated_gradients_resnet34_viaduct.png', np.uint8((output.permute(1, 2, 0).cpu().numpy() + 0.1) * image2_np))
    ...
    output = output.squeeze(0).abs()
    output /= output.max()
    cv2.imwrite('./assets/integrated_gradients_vgg19_fireboat.png', np.uint8((output.permute(1, 2, 0).cpu().numpy() + 0.1) * image_np))

    output = ig2.attribute(input_tensor2.unsqueeze(0))
    output = output.squeeze(0).abs()
    output /= output.max()
    cv2.imwrite('./assets/integrated_gradients_vgg19_viaduct.png', np.uint8((output.permute(1, 2, 0).cpu().numpy() + 0.1) * image2_np))